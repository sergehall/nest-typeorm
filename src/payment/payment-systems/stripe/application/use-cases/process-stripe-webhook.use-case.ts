...
export class ProcessStripeWebHookUseCase implements ICommandHandler<ProcessStripeWebHookCommand> {
  ...
  async execute(command: ProcessStripeWebHookCommand): Promise<boolean> {
    ...
    const event: StripeEvent | undefined = await this.commandBus.execute(
      ...
      if (event) {
        switch (event.type) {
          case 'checkout.session.completed':
            await this.commandBus.execute(new FinalizeStripePaymentCommand(event.data.object));
            break;
          case 'charge.succeeded':
            await this.commandBus.execute(new ProcessChargeSucceededCommand(event));
            break;

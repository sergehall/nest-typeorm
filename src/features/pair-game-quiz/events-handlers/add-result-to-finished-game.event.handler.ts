...
export class AddResultToFinishedGameEventHandler implements IEventHandler<GameOverEvent> {
  constructor(protected commandBus: CommandBus) {}

  async handle(event: GameOverEvent): Promise<boolean> {
    return await this.commandBus.execute(new AddResultToPairGameCommand(event.game));
  }
}

  ...
  async blogsSubscribersStatusCount(
    ...
    try {
      return await this.blogsSubscribersRepository
        ...
        .select([
          'blog.id AS "blogId"',
          `(SELECT COALESCE(COUNT(subscribers.id)::int, 0) FROM "nt-BlogsSubscribers" subscribers WHERE subscribers.blogId = blog.id AND subscribers.subscriptionStatus = :subscriptionStatus) AS "subscribersCount"`,
          `(SELECT COALESCE(MAX(subscribers.subscriptionStatus), :defaultStatus) FROM "nt-BlogsSubscribers" subscribers WHERE subscribers.blogId = blog.id AND subscribers.subscriber.userId = :subscriberId) AS "currentUserSubscriptionStatus"`,
        ])
